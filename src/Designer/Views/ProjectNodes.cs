...
namespace FreeSCADA.Designer.Views.ProjectNodes
{
	...
	class ScriptNode : BaseEntityNode
	{
        ...
        public override bool CanRename
        {
            get
            {
                return this.Name != "global" && this.Name != "functions" && this.Name != "channelhandlers" && !Env.Current.Project.ContainsEntity(ProjectEntityType.Schema, this.entityName);
            }
        }
        public override void Rename(string newName, System.Windows.Forms.TreeNode treeNode)
        {
            if (Env.Current.Project.RenameEntity(ProjectEntityType.Script, this.Name, newName))
            {
                this.entityName = newName;
                base.UpdateTreeNode(treeNode);
            }
        }
	}
}
