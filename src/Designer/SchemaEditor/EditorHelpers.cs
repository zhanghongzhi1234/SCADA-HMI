...
namespace FreeSCADA.Designer.SchemaEditor
{
    ...
    static class EditorHelper
    {
        ...
        public static Rect CalculateBounds(List<UIElement> elList, UIElement parent)
        {
            ...
            foreach (UIElement current in elList)
            {
                System.Windows.Rect rect = current.TransformToVisual(parent).TransformBounds(VisualTreeHelper.GetDescendantBounds(current));
                if (result.IsEmpty)
                {
                    result = rect;
                }
