...
namespace FreeSCADA.Designer.Views
{
    ...
    class ToolBoxView : ToolWindow
    {
        ...
        {
             TabText = "工具箱";
//            TabText = ResourceManager.GetString("ToolBox", resourceCulture);
        ...
        public void OnSetCurrentTool(ToolDescriptor toolToSet)
        {
            ...
            {   //set to selection tool
                ToolBoxTab tbt = toolBox.SelectedTab;
                ToolBoxItem tbi = tbt[0];
                tbi.Selected = true;
                tbt.SelectedItem = tbi;
                ToolChanged(tbi, null);
            }
        ...
        public void OnToolsCollectionChanged(List<ToolDescriptor> tools, Type currentTool)
        {
            ...
            if (tools != null)
            {
                ...
                foreach (ToolDescriptor tool in tools)
                {
                    ...
                    tbi.Selected = false;
                    if (tool.ToolType == currentTool && tbtoselect == null)
                    {
                        tbi.Selected = true;
                        tbtoselect = tbt;
                        tbitoselect = tbi;
                ...
                //Set default selected tab and default select item for every tab
                foreach (ToolBoxTab toolBoxTab2 in this.toolBox.Tabs)
                {
                    toolBoxTab2.SelectedItem = toolBoxTab2[0];
                }
                this.toolBox[0].Selected = true;
            }
            ResumeLayout(false);
            /*if (tbtoselect != null)
            {
