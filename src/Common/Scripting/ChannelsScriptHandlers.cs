...
{
	public class ChannelsScriptHandlers : IChannelsScriptHandlers
	{
        ...
        public void Clear()
        {
            foreach (KeyValuePair<string, Dictionary<string, ScriptCallInfo>> pair in associations)
            {
                pair.Value.Clear();
            }
            associations.Clear();
			...
			foreach (XmlElement channelNode in channelNodes)
			{
				...
				if (eventNodes.Count > 0)
				{
					...
					foreach (XmlElement eventNode in eventNodes)
					{
                        ...
                        ci.ClassName = eventNode.GetAttribute("className");
		...
		internal void Save()
		{
			...
			foreach (string channelName in associations.Keys)
			{
				...
				foreach (string eventName in associations[channelName].Keys)
				{
					...
					eventElem.SetAttribute("classname", associations[channelName][eventName].ClassName);
