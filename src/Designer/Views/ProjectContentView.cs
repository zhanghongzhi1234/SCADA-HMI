...
namespace FreeSCADA.Designer.Views
{
	class ProjectContentView:ToolWindow
    {
        ...
        /// <summary>Occurs when user rename on a node for schema from the list</summary>
        public event System.EventHandler SchemaRename;
		...
		public ProjectContentView()
		{
			...
			Env.Current.CommunicationPlugins.ChannelsChanged += new ChannelsChangedHandler(OnCommunicationPluginsChannelsChanged);
		...
		void OnRenameNodeClick(object sender, EventArgs e)
		{
            ...
            TreeNode treeNode = toolStripItem.Tag as TreeNode;
            BaseEntityNode baseEntityNode = treeNode.Tag as BaseEntityNode;
            RenameSchemaForm renameSchemaForm = new RenameSchemaForm(baseEntityNode.Name);
            if (renameSchemaForm.ShowDialog(this) == DialogResult.OK)
			{
                NodeRename nodeRename = new NodeRename();
                nodeRename.oldName = baseEntityNode.Name;
                nodeRename.newName = renameSchemaForm.SchemaName;
                if (baseEntityNode is SchemaNode)
                {
                    nodeRename.ftype = ProjectEntityType.Schema;
                }
                else
                {
                    if (baseEntityNode is ScriptNode)
                    {
                        nodeRename.ftype = ProjectEntityType.Script;
                    }
                }
                baseEntityNode.Rename(renameSchemaForm.SchemaName, treeNode);
                if (this.SchemaRename != null)
                {
                    this.SchemaRename(nodeRename, new System.EventArgs());
                }
			}
		}
